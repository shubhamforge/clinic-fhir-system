...
When(
  'vitals are recorded for that encounter:',
  async function (this: SeedWorld, dataTable: DataTable) {
    ...
    if (!this.currentEncounterId)
      throw new Error('No currentEncounterId — encounter step must run first');
    ...
    if (row['systolicBp'] !== undefined)
      body.systolicBp = parseFloat(row['systolicBp']);
    if (row['diastolicBp'] !== undefined)
      body.diastolicBp = parseFloat(row['diastolicBp']);
    if (row['weightKg'] !== undefined)
      body.weightKg = parseFloat(row['weightKg']);
    if (row['spo2Percent'] !== undefined)
      body.spo2Percent = parseFloat(row['spo2Percent']);
    if (row['heartRateBpm'] !== undefined)
      body.heartRateBpm = parseFloat(row['heartRateBpm']);
    if (row['temperatureCelsius'] !== undefined)
      body.temperatureCelsius = parseFloat(row['temperatureCelsius']);
