...
- [ ] Add HAPI FHIR dependencies to `pom.xml`
  ```xml
  <dependency>
  ...
  ```
- [ ] Run HAPI FHIR + PostgreSQL via `infra/docker-compose.yml`
- [ ] Verify: `GET http://localhost:8080/fhir/metadata` returns CapabilityStatement
- [ ] Implement `FhirConfig` → wire `IGenericClient` bean
- [ ] Implement `PatientController` → `PatientService` → `PatientMapper`
- [ ] Test: POST patient, GET patient by ID via Postman

---

## Phase 2 — Core Clinical (Days 3–5)

- [ ] Implement `EncounterController` → `EncounterService` → `EncounterMapper`
- [ ] Implement `VitalsController` → `VitalsService` → `ObservationMapper`
  - Support all 4 vitals: BP systolic, BP diastolic, weight, SpO2
  - Use correct LOINC codes (see `03-fhir-mapping.md`)
- [ ] Link vitals to an encounter via `Observation.encounter` reference
...
- [ ] Implement `SummaryService` (3-call FHIR aggregation)
- [ ] Add `GlobalExceptionHandler` (404, FHIR errors, validation errors)
- [ ] Add patient search endpoint (`?name=&dob=`)
- [ ] Add `application.properties` with configurable `fhir.server.url`
- [ ] Write `README.md` quick-start instructions
